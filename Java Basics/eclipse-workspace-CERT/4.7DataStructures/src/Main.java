...
public class Main {
    ...
    public static String findMostRepeatedWord(String s) {
        // Words are Keys	|	Counts are Values
    	//Keys cannot be repeated but values can
        HashMap<String, Integer> wordCountMap = new HashMap();

        // Normalize input with String methods
        ...
        //Iterate through key-set (words)
        for (String key : wordCountMap.keySet()) {
           
        	int currentValue = wordCountMap.get(key);
            
        	//keep track of most-repeated-word(key) and its max(value)
            if (currentValue > max) {
